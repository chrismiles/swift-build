        ...
        if context.settings.globalScope.evaluate(BuiltinMacros.GENERATE_PLAYGROUND_ENTRY_POINT) {
            await self.appendGeneratedTasks(&tasks) { delegate in
                ...
                for directDependency in context.globalProductPlan.dependencies(of: configuredTarget) {
                    ...
                    // TODO: *CM*
                    guard settings.productType?.conformsTo(identifier: "com.apple.product-type.bundle.unit-test") == false else {
                        continue
                    }
...
//                    indexStoreDirectories.append(path)
                ...
                await context.playgroundEntryPointGenerationToolSpec.constructTasks(cbc, delegate, indexStorePaths: indexStoreDirectories.elements, indexUnitBasePaths: indexUnitBasePaths.elements)
            }
        }
        return tasks
    }
